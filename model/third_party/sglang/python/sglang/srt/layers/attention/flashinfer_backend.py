    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            if self.is_multimodal or self.multi_item_scoring_delimiter is not None:
                if self.is_dllm_model and forward_batch.forward_mode.is_dllm_extend():
                    # dLLM block decode: use ragged wrapper for block self-attention
                    # and paged wrapper for prefix KV only.  Without ragged, the
                    # paged wrapper covers seq_lens (prefix+block), causing block
                    # tokens to attend to stale KV cache entries from prior
