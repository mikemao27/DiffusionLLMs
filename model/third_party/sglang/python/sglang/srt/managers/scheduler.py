...
class Scheduler(
    ...
    def init_chunked_prefill(self):
        ...
        if self.dllm_config is not None:
            if self.model_config.is_multimodal:
                # Multimodal dLLM: keep default chunked_prefill_size for
                # prompt prefill. Block diffusion uses block_size naturally
                # since _init_fill_ids_for_dllm appends block_size masks.
                pass
            else:
                # Text-only dLLM: use block_size for chunked prefill
        ...
        if batch.forward_mode.is_decode():
            ...
            if batch.is_dllm():
                if batch.forward_mode.is_dllm_extend():
                    ...
                    # Multimodal dLLM prompt prefill: process as prefill but
                    # keep request as chunked so next round adds mask tokens.
                    self.process_batch_result_dllm_prefill(batch, result)
            else:
