...
Measures INT8 weight quantization's speedup in isolation, and stacked with block eviction, as a confirmatory test of the profiling-derived diagnosis that decode is
bottlenecked on reading model weight bytes (GEMM-dominated), not on attention/KV cache, rather than as a test of the eviction kernel itself. See kernel/test_eviction.py and
kernel/profile_eviction.py for the eviction-only characterization this builds on.

Three configs are run back-to-back in one process so the comparison is self-contained and uses the same warm-up-corrected methodology as the rest of this project's
benchmarks: bf16 with no eviction as the baseline, INT8 with no eviction to isolate quantization's own effect, and INT8 + eviction (K = 16) to test whether the two stack
multiplicatively, as predicted (~1.3x attention x ~1.6x GEMM).

Requires bitsandbytes (pip install --break-system-packages bitsandbytes) and a recent enough transformers/accelerate for BitsAndBytesConfig support with a custom
PreTrainedModel subclass: this is the first time this codebase has gone through the HF quantization loading path, so treat the very first run as a smoke test of that
integration, not just of the speedup number.

Run from KINETIC/:

    CUDA_VISIBLE_DEVICES=0 python -m kernel.test_quantization \\ --n_questions 80 --batch_size 32 --context_len 2048 --k 16
"""
...
def load_model_variant(model_path, device, quantize):
    """
    Loads Fast-dLLM v2 either in plain bf16 or with INT8 weight quantization. model_path is the HuggingFace repo ID or local path, and device is the target CUDA device
    string. quantize is "none" for bf16, or "int8" for 8-bit weight quantization via bitsandbytes. Returns the loaded model, in eval mode, with .mdm_sample bound.
    """
    if quantize == "int8":
        # llm_int8_skip_modules leaves lm_head in bf16: quantizing the final projection to the vocabulary is a common source of avoidable accuracy loss for
        # comparatively little memory/speed benefit, since it's a single matmul rather than one per layer.
...
def evaluate_config(model, tokenizer, device, batches, k, union_mode, max_new_tokens, threshold, temperature):
    ...
    model is a model instance with mdm_sample bound, tokenizer is the model tokenizer, and device is the CUDA device string. batches is a list of (encoded_batch, answers)
    tuples from load_gsm8k_batched. k is the number of prefix blocks to retain, or None to run the full, unpruned prefix, and union_mode is the union strategy passed to
    BlockEvictionScheduler when k is set. max_new_tokens is the maximum tokens to generate per question, threshold is the unmasking confidence threshold, and
...
def run_one_config(label, model_path, device, quantize, k, args):
    """
    Loads a fresh model for one config, runs the eval, and tears the model down before returning. Each config gets its own process-level state rather than reusing a loaded
    model, since switching quantization on a live model isn't supported, and this keeps every config's warm-up cost handled identically and independently.
...
def main():
    ...
    # Illustrative comparison against the naive multiplicative prediction (eviction-only speedup, measured separately in test_eviction.py, times INT8-only speedup measured just above). 
    # Not a claim that the two effects are perfectly independent: just a sanity check on whether the stacked result is in the right neighborhood of that estimate.
