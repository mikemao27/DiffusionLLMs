...
Job: answer "where is the remaining gap between measured speedup and the
theoretical eviction ceiling actually going?" with a real torch.profiler
trace, instead of guessing between the commit-step scoring cost and the
S_q=32 fallback path.

Run from DiffusionLLMs/:
    CUDA_VISIBLE_DEVICES=0 python -m kernel.profile_eviction --k 16 --n_questions 16

Interpreting the output:
    - Look at the printed table's top rows by self CUDA time. Any op named
      like "aten::scaled_dot_product_attention" attributable to score-capture
      (paired with a matmul/softmax done in eager mode rather than fused) is
      the commit-step double-attention cost.
    - Time in "aten::index" / "aten::cat" / "aten::repeat_interleave" ahead of
      "aten::scaled_dot_product_attention" is the gather+cat+repeat overhead
      in the S_q=32 fallback path.
    - The exported chrome trace (profile_trace.json) can be loaded at
      chrome://tracing or https://ui.perfetto.dev for a visual timeline if the
      text table isn't enough to localize the cost.
"""
