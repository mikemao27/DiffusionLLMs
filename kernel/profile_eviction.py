...
Interpreting the output: look at the printed table's top rows by self CUDA time. Any op named like "aten::scaled_dot_product_attention" attributable to 
score-capture (paired with a matmul/softmax done in eager mode rather than fused) is the commit-step double-attention cost. Time in "aten::index" / 
"aten::cat" / "aten::repeat_interleave" ahead of "aten::scaled_dot_product_attention" is the gather + cat + repeat overhead in the S_q = 32 fallback 
path. The exported chrome trace (profile_trace.json) can be loaded at chrome://tracing or https://ui.perfetto.dev for a visual timeline if the text 
table isn't enough to localize the cost.
"""
...
def main():
    ...
    # commit_step / small_step are record_function wrapper scopes (added in BlockEvictionScheduler._patched_forward), not real ops: almost all of their 
    # own time is spent inside child ops, so they show near-zero "self" time and won't surface in the self-time table above no matter how expensive 
    # they are. Their cuda_time_total (which does include descendants) is the number that actually answers "which of these two dominates": printed here 
    # explicitly since the table above can't show it.
    print("\nCommit-step vs small-step breakdown (total CUDA time, including all nested ops):")
    tag_totals = {}
    for evt in prof.key_averages():
        if evt.key in ("commit_step", "small_step"):
            # Attribute name varies by torch version: newer versions generalized cuda_time_total to device_time_total as part of supporting non-CUDA
