        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_value is not None:
            ...
            if update_past_key_values:
                ...
                if _idx is not None:
                    _idx = _idx[_idx < prefix_k.shape[-2]]
                    if len(_idx) < prefix_k.shape[-2]:
                        # actual pruning: gather selected tokens only
                        prefix_len = prefix_k.shape[-2]
                        prefix_k = prefix_k[:, :, _idx, :]
                        prefix_v = prefix_v[:, :, _idx, :]
                        # Slice the attention mask to match the pruned prefix length.
                        # mask shape: [..., prefix_len + block_cache_len]
                        # Keep prefix columns at _idx positions + all block-cache columns.
                    ...
                    # else: all tokens selected (K >= N) — skip gather to preserve
                    # memory layout and keep output bit-identical to baseline
